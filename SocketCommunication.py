...
class SocketCommuncication(Node):

    def __init__(self, ip, port):
        super(SocketCommuncication, self).__init__(ip, port, None)
        self.peers = []
        self.peerDiscoveryHandler = PeerDiscoveryHandler(self)
        self.socketConnector = SocketConnector(ip, port)

    def connectToFirstNode(self):
        if self.socketConnector.port != 1001:
            self.connect_with_node('localhost', 1001)
